...
func TestParseResourceID(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err: fmt.Errorf(
				`could not parse ID: "apiVersion=apps/v1,kind=Deployment,name=app,junk=test". ID contained unknown key "junk"`,
			),
		},
		{
			id: "apiVersion_apps/v1,kind=Deployment,name=app",
			err: fmt.Errorf(
				`could not parse ID: "apiVersion_apps/v1,kind=Deployment,name=app". ID must be in key=value format`,
			),
		},
		{
			id: "junk",
			err: fmt.Errorf(
				`could not parse ID: "junk". ID must contain apiVersion, kind, and name`,
			),
		},
	}
