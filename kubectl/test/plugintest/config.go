...
func DiscoverConfig(ctx context.Context, sourceDir string) (*Config, error) {
	...
	case tfPath != "":
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf("Adding potential Terraform CLI source of exact path: %s", tfPath),
		)
	...
	case tfVersion != "":
		...
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Adding potential Terraform CLI source of releases.hashicorp.com exact version %q for installation in: %s",
				tfVersion,
				tfDir,
			),
		)
	...
	default:
		logging.HelperResourceTrace(
			ctx,
			"Adding potential Terraform CLI source of local filesystem PATH lookup",
		)
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Adding potential Terraform CLI source of checkpoint.hashicorp.com latest version for installation in: %s",
				tfDir,
			),
		)
