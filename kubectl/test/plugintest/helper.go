...
func (h *Helper) NewWorkingDir(ctx context.Context, t TestControl) (*WorkingDir, error) {
	...
	if tfAccLog != "" {
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Setting terraform-exec log level via %s environment variable, if Terraform CLI is version 0.15 or later",
				EnvTfAccLog,
			),
		...
		if err != nil {
			if !errors.As(err, new(*tfexec.ErrVersionMismatch)) {
				...
				return nil, fmt.Errorf(
					"unable to set terraform-exec log level (%s): %w",
					tfAccLog,
					err,
				)
			}
			...
			logging.HelperResourceWarn(
				ctx,
				fmt.Sprintf(
					"Unable to set terraform-exec log level via %s environment variable, as Terraform CLI is version 0.14 or earlier. It will default to TRACE.",
					EnvTfAccLog,
				),
	...
	if tfLogCore != "" {
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Setting terraform-exec core log level via %s environment variable, if Terraform CLI is version 0.15 or later",
				EnvTfLogCore,
			),
			map[string]any{
		...
		if err != nil {
			...
			return nil, fmt.Errorf(
				"unable to set terraform-exec core log level (%s): %w",
				tfLogCore,
				err,
			)
		}
	...
	if tfLogProvider != "" {
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Setting terraform-exec provider log level via %s environment variable, if Terraform CLI is version 0.15 or later",
				EnvTfLogProvider,
			),
			map[string]any{
		...
		if err != nil {
			...
			return nil, fmt.Errorf(
				"unable to set terraform-exec provider log level (%s): %w",
				tfLogProvider,
				err,
			)
		}
	...
	if logPath != "" {
		logging.HelperResourceTrace(
			ctx,
			fmt.Sprintf(
				"Setting terraform-exec log path via %s environment variable",
				logPathEnvVar,
			),
