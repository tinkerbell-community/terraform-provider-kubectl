...
func TestKubernetesManifest_Import(t *testing.T) {
	...
	defer k8shelper.DeleteResource(
		t,
		namespace,
		kubernetes.NewGroupVersionResource("v1", "namespaces"),
	)
	...
	if len(plan.ResourceChanges) != 1 ||
		plan.ResourceChanges[0].Address != "kubernetes_manifest.test" {
		t.Fatalf("Failed to find resource in plan data: %q", plan.ResourceChanges[0].Address)
	}
	if len(plan.ResourceChanges[0].Change.Actions) != 1 ||
		plan.ResourceChanges[0].Change.Actions[0] != "update" {
		t.Fatalf(
			"Failed to plan for resource update - in fact, planned for: %q",
			plan.ResourceChanges[0].Change.Actions[0],
		)
	}
