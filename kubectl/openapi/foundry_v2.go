...
func (f *foapiv2) getTypeByID(
	...
	srf := openapi2conv.ToV3SchemaRef(swd)
	definitions := openapi2conv.ToV3Schemas(f.swagger.Definitions)

	sch, err := resolveSchemaRef(srf, definitions)
	if err != nil {
		return nil, fmt.Errorf("failed to resolve schema: %s", err)
	}

	return getTypeFromSchema(sch, f.recursionDepth, &(f.typeCache), definitions, ap, h)
}
...
func (f *foapiv2) buildGvkIndex() error {
	for did, dRef := range f.swagger.Definitions {
		...
		definitions := openapi2conv.ToV3Schemas(f.swagger.Definitions)
		def, err := resolveSchemaRef(sRef, definitions)
		if err != nil {
			return err
		}
