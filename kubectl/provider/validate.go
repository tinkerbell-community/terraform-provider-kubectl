	...
	for _, key := range requiredKeys {
		if _, present := rawManifest[key]; !present {
			...
			resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
				...
				Detail: fmt.Sprintf(
					"'%s' attribute key is missing from manifest configuration",
					key,
				),
				Attribute: kp,
			})
	...
	for _, key := range forbiddenKeys {
		if _, present := rawManifest[key]; present {
			...
			resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
				...
				Detail: fmt.Sprintf(
					"'%s' attribute key is not allowed in manifest configuration",
					key,
				),
				Attribute: kp,
			})
...
func (s *RawProviderServer) validateResourceOnline(
	manifest *tftypes.Value,
	...
	// during 'ValidateResourceConfig', but at that point we don't have access to API credentials
		...
		if err != nil || len(restPath.Steps()) > 0 {
			diags = append(diags,
				&tfprotov6.Diagnostic{
					Severity: tfprotov6.DiagnosticSeverityError,
					Detail: fmt.Sprintf(
						"Resources of type '%s' require a namespace",
						gvk.String(),
					),
		...
		if nsVal.(tftypes.Value).IsNull() {
			diags = append(diags,
				&tfprotov6.Diagnostic{
					Severity: tfprotov6.DiagnosticSeverityError,
					Detail: fmt.Sprintf(
						"Namespace for resource '%s' cannot be nil",
						gvk.String(),
					),
					Summary: "Namespace required",
				})
		...
		if nsStr == "" && err == nil {
			diags = append(diags,
				&tfprotov6.Diagnostic{
					Severity: tfprotov6.DiagnosticSeverityError,
					Detail: fmt.Sprintf(
						"Namespace for resource '%s' cannot be empty",
						gvk.String(),
					),
					Summary: "Namespace required",
				})
