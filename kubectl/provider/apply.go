...
func (s *RawProviderServer) ApplyResourceChange(
	...
	if ok && !cfVal.IsNull() && cfVal.IsKnown() {
		...
		for _, v := range cf {
			...
			if err != nil {
				s.logger.Error(
					"[Configure]",
					"[computed_fields] cannot parse field path element",
					err,
				)
				resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
					Severity: tfprotov6.DiagnosticSeverityError,
	...
	case applyPriorState.IsNull() || (!applyPlannedState.IsNull() && !applyPriorState.IsNull()):
		...
		if err != nil {
			resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
				...
				Detail: fmt.Sprintf(
					`This can happen when the "apiVersion" or "kind" fields are not present in the manifest, or when the corresponding "kind" or "apiVersion" could not be found on the Kubernetes cluster.\nError: %s`,
					err,
				),
			})
			return resp, nil
		...
		rqObj := mapRemoveNulls(pu.(map[string]any))
			...
			if apierrors.IsConflict(err) {
				resp.Diagnostics = append(resp.Diagnostics,
					&tfprotov6.Diagnostic{
						Severity: tfprotov6.DiagnosticSeverityError,
						Summary: fmt.Sprintf(
							`There was a field manager conflict when trying to apply the manifest for %q`,
							rnn,
						),
						Detail: fmt.Sprintf(
		...
		if wf, ok := plannedStateVal["wait_for"]; ok && !wf.IsNull() {
			s.logger.Trace(
				"[ApplyResourceChange][Wait] Using waiter config from deprecated `wait_for` attribute",
			)
			waitConfig = wf
		}
		if !waitConfig.IsNull() {
			...
			if err != nil {
				s.logger.Error(
					"[ApplyResourceChange][ReadAfterWait]",
					"API error",
					dump(err),
					"API response",
					dump(result),
				...
				resp.Diagnostics = append(resp.Diagnostics,
					&tfprotov6.Diagnostic{
						Severity: tfprotov6.DiagnosticSeverityError,
						Summary: fmt.Sprintf(
							`Failed to read resource %q after wait conditions`,
							rname,
						),
						Detail: err.Error(),
					})
		...
		newResObject, err := payload.ToTFValue(
			RemoveServerSideFields(result.Object),
			tsch,
			th,
			tftypes.NewAttributePath(),
		)
	...
	case applyPlannedState.IsNull():
		...
		uo := unstructured.Unstructured{Object: pu.(map[string]any)}
		gvr, err := GVRFromUnstructured(&uo, m)
		if err != nil {
			return resp, err
		}
			...
			if apierrors.IsNotFound(err) {
				...
				resp.Diagnostics = append(resp.Diagnostics,
					&tfprotov6.Diagnostic{
						...
						Detail: fmt.Sprintf(
							"The resource %q was not found in the Kubernetes API. This may be due to the resource being already deleted.",
							rname,
						),
					})
			} else {
			...
			if time.Now().After(deadline) {
				resp.Diagnostics = append(resp.Diagnostics,
					&tfprotov6.Diagnostic{
						Severity: tfprotov6.DiagnosticSeverityError,
						Summary: fmt.Sprintf(
							"Timed out when waiting for resource %q to be deleted",
							rname,
						),
			...
			if err != nil {
				...
				resp.Diagnostics = append(resp.Diagnostics,
					&tfprotov6.Diagnostic{
						...
						Detail: fmt.Sprintf(
							"Error when waiting for resource %q to be deleted: %v",
							rname,
							err,
						),
					})
