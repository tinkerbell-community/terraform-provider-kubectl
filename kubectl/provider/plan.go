...
func (s *RawProviderServer) dryRun(
	ctx context.Context,
	obj tftypes.Value,
	fieldManager string,
	forceConflicts bool,
...
func (s *RawProviderServer) getFieldManagerConfig(
	...
	if !v["field_manager"].IsNull() && v["field_manager"].IsKnown() {
		...
		if len(fieldManagerBlock) > 0 {
			...
			if !fieldManagerObj["force_conflicts"].IsNull() &&
				fieldManagerObj["force_conflicts"].IsKnown() {
...
func (s *RawProviderServer) PlanResourceChange(
	...
	if canDeferr && s.clientConfigUnknown {
		...
		ps, err := tfprotov6.NewDynamicValue(newPlannedState.Type(), newPlannedState)
		if err != nil {
			return resp, err
		}
	...
	if ok && !cfVal.IsNull() && cfVal.IsKnown() {
		...
		for _, v := range cf {
			...
			if err != nil {
				s.logger.Error(
					"[Configure]",
					"[computed_fields] cannot parse filed path element",
					err,
				)
				resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
					Severity: tfprotov6.DiagnosticSeverityError,
	...
	if err != nil {
		...
		if canDeferr && meta.IsNoMatchError(err) {
			...
			rd.Severity = tfprotov6.DiagnosticSeverityWarning
		}
		return resp, nil
	}
	...
	if !objectType.Is(tftypes.Object{}) {
		...
		if err != nil {
			resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
				...
				Detail: fmt.Sprintf(
					"A dry-run apply was performed for this resource but was unsuccessful: %v",
					err,
				),
			})
			return resp, nil
	...
	morphedManifest, d := morph.ValueToType(
		ppMan,
		objectType,
		tftypes.NewAttributePath().WithAttributeName("object"),
	)
	...
	completePropMan, err := morph.DeepUnknown(
		objectType,
		morphedManifest,
		tftypes.NewAttributePath().WithAttributeName("object"),
	)
	if err != nil {
		resp.Diagnostics = append(resp.Diagnostics, &tfprotov6.Diagnostic{
			...
			Detail: fmt.Sprintf(
				"This usually happens when the provider cannot fully process the schema retrieved from cluster. Please report this to the provider maintainers.\nError: %s",
				err.Error(),
			),
