...
func ValueToType(
	...
	if t == nil {
		diags = append(diags, &tfprotov6.Diagnostic{
			...
			Detail: fmt.Sprintf(
				"Cannot convert value into 'nil' type at attribute: %s",
	...
	if !v.IsKnown() {
		diags = append(diags, &tfprotov6.Diagnostic{
			...
			Detail: fmt.Sprintf(
				"Type conversion cannot be performed on (partially) unknown value at attribute: %s",
				attributePathSummary(p),
			),
		})
		return v, diags
...
func morphBoolToType(
	...
	diags = append(diags, &tfprotov6.Diagnostic{
		...
		Summary:   "Value incompatible with expected type",
		Detail: fmt.Sprintf(
			"Cannot convert Bool values into type %s\n ...at attribute\n%s",
			typeNameNoPrefix(t),
			attributePathSummary(p),
		),
	})
...
func morphNumberToType(
	...
	diags = append(diags, &tfprotov6.Diagnostic{
		...
		Summary:   "Value incompatible with expected type",
		Detail: fmt.Sprintf(
			"Cannot convert Number values into type %s\n ...at attribute\n%s",
			typeNameNoPrefix(t),
			attributePathSummary(p),
		),
	})
...
func morphStringToType(
	...
	diags = append(diags, &tfprotov6.Diagnostic{
		...
		Summary:   "Value incompatible with expected type",
		Detail: fmt.Sprintf(
			"Cannot transform String value into type %s\n ...at attribute\n%s",
			typeNameNoPrefix(t),
			attributePathSummary(p),
		),
	})
...
func validateValue(t tftypes.Type, val any, p *tftypes.AttributePath) []*tfprotov6.Diagnostic {
	var diags []*tfprotov6.Diagnostic
