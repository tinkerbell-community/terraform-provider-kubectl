...
func FromTFValue(in tftypes.Value, th map[string]string, ap *tftypes.AttributePath) (any, error) {
	...
	case in.Type().Is(tftypes.Bool):
		...
		err = in.As(&bv)
		if err != nil {
			return nil, ap.NewErrorf(
				"[%s] cannot extract contents of attribute: %s",
				ap.String(),
				err,
			)
		}
	...
	case in.Type().Is(tftypes.Number):
		...
		if nv.IsInt() {
			...
			if acc != big.Exact {
				return nil, ap.NewErrorf(
					"[%s] inexact integer approximation when converting number value at:",
					ap.String(),
				)
			}
			return inv, nil
	...
	case in.Type().Is(tftypes.List{}) || in.Type().Is(tftypes.Tuple{}):
		...
		for k, le := range l {
			...
			if err != nil {
				return nil, nextAp.NewErrorf(
					"[%s] cannot convert list element to Unstructured: %s",
					nextAp.String(),
					err,
				)
			}
	...
	case in.Type().Is(tftypes.Map{}) || in.Type().Is(tftypes.Object{}):
		...
		for k, me := range m {
			...
			if err != nil {
				return nil, nextAp.NewErrorf(
					"[%s]: cannot convert map element to Unstructured: %s",
					nextAp.String(),
					err.Error(),
				)
			}
	...
	default:
		return nil, ap.NewErrorf(
			"[%s] cannot convert value of unknown type (%s)",
			ap.String(),
			in.Type().String(),
		)
	}
