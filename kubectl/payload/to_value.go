...
func ToTFValue(
	...
	case string:
		...
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "string" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case bool:
		...
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "bool" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case int:
		...
		case st.Is(tftypes.String):
			...
			if ok &&
				ht == "io.k8s.apimachinery.pkg.util.intstr.IntOrString" { // We store this in state as "string"
				return tftypes.NewValue(tftypes.String, strconv.FormatInt(int64(t), 10)), nil
			}
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int" to "tftypes.String"`,
				at.String(),
			)
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case int64:
		...
		case st.Is(tftypes.String):
			...
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int64" to "tftypes.String"`,
				at.String(),
			)
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int64" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case int32:
		...
		case st.Is(tftypes.String):
			...
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int32" to "tftypes.String"`,
				at.String(),
			)
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int32" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case int16:
		...
		case st.Is(tftypes.String):
			...
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "int16" to "tftypes.String"`,
				at.String(),
			)
		default:
			return tftypes.Value{}, at.NewErrorf(
	...
	case float64:
		...
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "float64" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case []any:
		...
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "[]interface{}" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	case map[string]any:
		...
		default:
			return tftypes.Value{}, at.NewErrorf(
				`[%s] cannot convert payload from "map[string]interface{}" to "%s"`,
				at.String(),
				st.String(),
			)
		}
	...
	return tftypes.Value{}, at.NewErrorf(
		`[%s] cannot convert payload of unknown type "%s"`,
		at.String(),
		reflect.TypeOf(in).String(),
	)
}
	...
	for k, v := range in {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] cannot convert list element '%s' as DynamicPseudoType",
				eap,
				err,
			)
		}
...
func sliceToTFListValue(
	...
	for k, v := range in {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] cannot convert list element value: %s",
				eap,
				err,
			)
		}
...
func sliceToTFTupleValue(
	...
	for k, v := range in {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] cannot convert list element [%d] to '%s': %s",
				eap.String(),
				k,
				et.String(),
				err,
	...
	for k, v := range in {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				...
				eap,
				k,
				st.(tftypes.Set).ElementType.String(),
				err,
			)
		}
		il[k] = iv
...
func mapToTFMapValue(
	...
	for k, v := range in {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] cannot convert map element '%s' to '%s': err",
				eap,
				st.(tftypes.Map).ElementType.String(),
				err,
			)
		}
		im[k] = mv
		...
		if !oType.Is(im[k].Type()) {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] conflicting map element type: %s",
				eap.String(),
				mv.Type(),
			)
		}
...
func mapToTFObjectValue(
	...
	for k, kt := range st.(tftypes.Object).AttributeTypes {
		...
		if err != nil {
			return tftypes.Value{}, eap.NewErrorf(
				"[%s] cannot convert map element value: %s",
				eap,
				err,
			)
		}
