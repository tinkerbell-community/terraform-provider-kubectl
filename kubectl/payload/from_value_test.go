...
func TestFromTFValue(t *testing.T) {
	...
	samples := map[string]struct {
		...
		"map-of-int-or-strings": {
			...
			Th: map[string]string{
				tftypes.NewAttributePath().WithElementKeyString("#").String(): "io.k8s.apimachinery.pkg.util.intstr.IntOrString",
			},
			Out: map[string]any{
				"foo": "test1",
				"bar": 42,
		...
		"object": {
			...
			Th: map[string]string{
				tftypes.NewAttributePath().WithAttributeName("stuff").String(): "io.k8s.apimachinery.pkg.util.intstr.IntOrString",
			},
			Out: map[string]any{
				"foo":    "bar",
				"stuff":  42,
